...
## Crediting model sources (required for CC-licensed models)

`credits.html` lists where each model came from and its license — required if you're using Creative Commons–licensed models from Sketchfab (or anywhere else). For every model, Sketchfab shows a ready-made attribution snippet on the model's page under **"License"** — it gives you the exact title, author name + profile link, and license name + link. Copy those straight into a `<div class="credit-entry">` block in `credits.html` (duplicate the existing block as a template).

If you modified a model (recolored, simplified, retextured), most CC licenses (like CC BY) require you to note that — there's a commented-out `<p class="mod-note">` line ready for that in each entry.

**Important:** some Sketchfab models are CC BY-NC (non-commercial only) or have other restrictions — double check the exact license on each model's page before using it, especially if this site will ever be used somewhere that could count as commercial.
...
4. If the model is CC-licensed, add a matching `<div class="credit-entry">` block to `credits.html`.
